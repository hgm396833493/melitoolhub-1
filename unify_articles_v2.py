...
def extract_auth_modal(content):
    """提取认证模态框"""
    pattern = r'<!-- ===== Auth Modal ===== -->.*?</div>\s*</div>\s*</body>'
    match = re.search(pattern, content, re.DOTALL)
    if not match:
        # 尝试其他模式
        pattern = r'<div class="auth-overlay".*?</div>\s*</div>'
        match = re.search(pattern, content, re.DOTALL)
    if match:
        return match.group(0)
    return ""

def extract_community_section(content):
    """提取评论区"""
    pattern = r'<!-- ===== Community Discussion Section ===== -->.*?</section>'
    match = re.search(pattern, content, re.DOTALL)
    if not match:
        pattern = r'<section class="community-section".*?</section>'
        match = re.search(pattern, content, re.DOTALL)
    if match:
        return match.group(0)
    return ""
...
def extract_rocket_button(content):
    """提取火箭按钮"""
    pattern = r'<!-- ===== 小火箭浮动按钮 ===== -->.*?</a>'
    match = re.search(pattern, content, re.DOTALL)
    if not match:
        pattern = r'<a href="https://h5n5.*?class="rocket-btn".*?</a>'
        match = re.search(pattern, content, re.DOTALL)
    if match:
        return match.group(0)
    return ""

def unify_file(filepath, reference_content):
    """统一单个文件"""
...
def main():
    ...
    for filename in FILES_TO_MODIFY:
        ...
        if unify_file(filepath, reference_content):
            success_count += 1
